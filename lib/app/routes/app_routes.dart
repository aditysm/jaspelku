part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();
  static const HOME = _Paths.HOME;
  static const LOGIN = _Paths.LOGIN;
  static const REGISTER = _Paths.REGISTER;
  static const REGISTER_ROLE = _Paths.REGISTER_ROLE;
  static const MAIN_PAGE = _Paths.MAIN_PAGE;
  static const LAYANAN = _Paths.LAYANAN;
  static const POSTINGAN_BARU = _Paths.POSTINGAN_BARU;
  static const PESAN = _Paths.PESAN;
  static const PROFIL = _Paths.PROFIL;
  static const PENGATURAN = _Paths.PENGATURAN;
  static const PENCARIAN = _Paths.PENCARIAN;
  static const HISTORI_PESANAN = _Paths.HISTORI_PESANAN;
  static const ULASAN_PESANAN = _Paths.ULASAN_PESANAN;
  static const CHAT_ROOM = _Paths.CHAT_ROOM;
  static const EDIT_PROFIL = _Paths.EDIT_PROFIL;
  static const NOTIFIKASI = _Paths.NOTIFIKASI;
  static const PEMBAYARAN_PESANAN = _Paths.PEMBAYARAN_PESANAN;
  static const PENGENALAN = _Paths.PENGENALAN;
}

abstract class _Paths {
  _Paths._();
  static const HOME = '/home';
  static const LOGIN = '/login';
  static const REGISTER = '/register';
  static const REGISTER_ROLE = '/register-role';
  static const MAIN_PAGE = '/main-page';
  static const LAYANAN = '/layanan';
  static const POSTINGAN_BARU = '/postingan-baru';
  static const PESAN = '/pesan';
  static const PROFIL = '/profil';
  static const PENGATURAN = '/pengaturan';
  static const PENCARIAN = '/pencarian';
  static const HISTORI_PESANAN = '/histori-pesanan';
  static const ULASAN_PESANAN = '/ulasan-pesanan';
  static const CHAT_ROOM = '/chat-room';
  static const EDIT_PROFIL = '/edit-profil';
  static const NOTIFIKASI = '/notifikasi';
  static const PEMBAYARAN_PESANAN = '/pembayaran-pesanan';
  static const PENGENALAN = '/pengenalan';
}
